    ...
    <AppShell breadcrumb="Controle / Modelos">
      <div className="p-8 max-w-6xl mx-auto space-y-8">
        <header className="space-y-2">
          ...
          <p className="text-xs text-white/20 uppercase tracking-[0.35em] font-mono">Cadastro persistido diretamente na tabela modelo</p>
        </header>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
          <form onSubmit={handleCreate} className="bg-white/[0.02] border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl">
            <div className="space-y-2">
              <p className="text-sm font-black text-white uppercase tracking-widest">Novo modelo</p>
              <p className="text-sm text-white/40 font-mono">O codigo precisa ser unico e vai direto para o banco.</p>
            </div>

            <label className="block space-y-2">
              <span className="text-xs uppercase font-black text-white/60 tracking-widest">Codigo do modelo</span>
              <input
                value={codigo}
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="w-full py-3 bg-fuchsia-500 text-black font-mono text-sm font-black uppercase rounded-xl hover:bg-fuchsia-400 disabled:opacity-50 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              onClick={loadModels}
              disabled={isLoading}
              className="w-full py-3 bg-white/5 border border-white/10 text-white font-mono text-sm font-black uppercase rounded-xl hover:border-fuchsia-500 transition-all disabled:opacity-50 cursor-pointer"
            >
              Atualizar listagem
            </button>

            <div className="pt-2 border-t border-white/10 space-y-1">
              <p className="text-xs uppercase font-black text-white/60 tracking-widest">Resumo</p>
              <p className="text-sm font-mono text-white/40">Modelos cadastrados: {isLoading ? '-' : models.length}</p>
              <p className="text-sm font-mono text-white/40">Placas vinculadas: {isLoading ? '-' : totalPlacas}</p>
              {error && <p className="text-sm font-mono text-red-300">{error}</p>}
            </div>
          </form>
          ...
          <div className="bg-white/[0.02] border border-white/10 rounded-3xl overflow-hidden shadow-xl">
            <div className="flex items-center justify-between gap-4 px-6 py-4 border-b border-white/10 bg-white/[0.02]">
              <div>
                <p className="text-sm font-black text-white uppercase tracking-widest">Modelos persistidos</p>
                <p className="text-sm text-white/40 font-mono">Listagem diretamente do banco de dados</p>
              </div>
              <p className="text-xs font-mono text-white/40 uppercase">{isLoading ? 'Carregando...' : `${models.length} registros`}</p>
            </div>
